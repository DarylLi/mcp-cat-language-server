...
export class CatTranslator {
  private readonly catSounds = ["喵", "喵喵", "喵～", "喵喵喵"];
  ...
  private generateCatLanguage(text: string, charCount: number): string {
    // 根据文本内容和长度生成个性化的猫语
    let catPhrase = "";
  ...
  private generateQuestionCat(charCount: number): string {
  ...
  private generateExcitedCat(charCount: number): string {
    const baseCount = Math.min(Math.max(charCount, 2), 5);
    let result = "";
  ...
  private generateStatementCat(charCount: number): string {
